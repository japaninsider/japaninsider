  ...
  if (form) {
    ...
    async function copyReceiptText() {
      ...
      try {
        ...
        if (receiptActionStatus) receiptActionStatus.textContent = ja ? 'コピーできませんでした。印刷 / PDF保存をご利用ください。' : 'Could not copy. Please use Print / Save PDF.';
      }
    }
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        if (status) status.textContent = ja
          ? '送信しました。下の送信控えを保存してください。'
          : 'Sent. Please save the submission receipt below.';
      } catch (_) {
        if (status) status.textContent = ja
